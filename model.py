...
#    reader = csv.reader(file)
    ...
    def __init__(self, N_customers=2, width= 20, height = 20, vaccination_prop=0, avoid_radius=3):
        ...
        grid = []
        with open("albert.csv", encoding='utf-8-sig', newline = "") as file:
           reader = csv.reader(file)
    ...
    def new_obstacle(self, x, y, type_id):
        """Adds a new agent as obstacle to a random location on the grid. Returns the created agent"""
        pos = (x, y)

        new_agent = agent.Obstacle(type_id, self, pos)
