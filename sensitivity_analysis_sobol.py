...
def saver(dictex, var_name):
    for key, val in dictex.items():
        val.to_csv("results/sobol/data_{}.csv".format(str(key)))

    with open("results/sobol/keys_{}.txt".format(str(key)), "w") as f: #saving keys to file
        f.write(str(list(dictex.keys())))

# https://stackoverflow.com/questions/50786266/writing-dictionary-of-dataframes-to-file
def loader(var_name):
    """Reading data from keys"""
    with open("results/sobol/keys_{}.txt".format(var_name), "r") as f:
        keys = eval(f.read())

    dictex = {}
    for key in keys:
        dictex[key] = pd.read_csv("results/sobol/data_{}.csv".format(str(key)))
...
def main():
    ...
    for i, vals in enumerate(params):
        ...
        print("\nProgress: {:.2%}".format(float(i)/len(params)))
